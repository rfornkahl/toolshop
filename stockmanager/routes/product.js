...
router.post('/add', auth.authenticationToken,checkRole.checkRole,(req,res)=>{
    ...
    var query = "insert into product (name,categoryId, description, price, quantity, status) values(?,?,?,?,?,'true')";
...
router.get("/get", auth.authenticationToken, (req,res,next)=>{
    var query = "select p.id,p.name,p.description,p.price,p.status, p.quantity, c.id as categoryId,c.name as categoryName from product as p INNER JOIN category as c where p.categoryID = c.id";
...
router.get('/getById/:id', auth.authenticationToken,(req,res,next)=>{
    const id = req.params.id;
    var query = "select id,name,description,price, quantity from product where id=?";
    connection.query(query,[id],(err,results)=>{
        if (!err){
...
router.patch('/update', auth.authenticationToken,checkRole.checkRole ,(req,res,next)=>{
    let product = req.body;
    var query = "update product set name=?, categoryId=?,description=?,price=?, quantity=? where id=?";
...
router.patch('/updateQuantity', auth.authenticationToken,checkRole.checkRole ,(req,res,next)=>{
    let product = req.body;
    var query = "update product set quantity=? where id=?";
...
router.patch('/updateStatus',auth.authenticationToken,checkRole.checkRole ,(req,res,next)=>{
    ...
    connection.query(query,[user.status,user.id],(err,results)=>{
        if(!err){
            ...
            return res.status(200).json({message: "Product status has been updated successfully"});
        }
        else{
            return res.status(500).json(err);
        }
